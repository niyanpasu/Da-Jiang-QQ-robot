...
string runCpp(const string & data)
{
	try
	{
		string arg = "code=" + escape(GBKToUTF8(data)) + "&language=7&fileext=cpp";
		string str = curlpost("https://m.runoob.com/api/compile.php", arg.c_str());
		if (str.find(R"(output":null,)") != -1)return string{ "运行时间太长,大酱不想跑了,滚去优化^_~" };
		Json::Value root;  
		std::istringstream os(str);
		os >> root;
		std::string output = root["output"].asString(); 
		std::string errors = root["errors"].asString();
		return UTF8ToGBK(output + (errors.length() > 1 ? errors : ""));
	}
	catch (...)
	{
		return "大酱发生异常了_(:3」∠)_";
...
CQEVENT(int32_t, __eventPrivateMsg, 24)(int32_t subType, int32_t msgId, int64_t fromQQ, const char *msg, int32_t font) {
	...
	{
		//消除转义
		replace_all_distinct(vec[2], "&amp;", "&");
		replace_all_distinct(vec[2], "&#91;", "[");
		replace_all_distinct(vec[2], "&#93;", "]");
		replace_all_distinct(vec[2], "&#44;", ",");

		//获取运行结果
		string ret = runCpp(vec[2]);
